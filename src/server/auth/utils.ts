 ...
 */
export async function linkAnonymousUser(anonymousUserId: string, newUserId: string) {
    try {
        await db.$transaction(async (tx) => {
            ...
            // Re-associate custom drinks
            await tx.drink.updateMany({
                where: { createdByUserId: anonymousUserId },
                data: { createdByUserId: newUserId },
            });
            ...
            const anonUser = await tx.user.findUnique({ where: { id: anonymousUserId } });
            if (anonUser) {
        ...
        // Check if it's a foreign key constraint error (new user doesn't exist)
